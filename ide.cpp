  ...
  void init(){
    ...
    if(stt&1){
      cns->puts("HD not found\n");
        ...
        if(!(sendcmd(0, 0, 0, 0, 0xa0, 0xa0)&1)){
          ...
          for(int i=0;i<4*2/2;i++)*(unsigned short*)&p[i*2]=io_in16(0x1f0);
          for(int i=0;i<4;i++){
            b[i]=d[3-i];
            b[i+4]=d[7-i];
          }
          ...
          idecdrv* drv=new idecdrv(0);
          drv->bpb=bpb;
          if(lba<0xffffffff)drvd::registdrv(1, 0, 0, (drive*)drv);
        }else{
      ...
      if(*(unsigned int*)&ide->identd[0x78]<0xfffffff&&*(unsigned int*)&ide->identd[0x78])drvd::registdrv(1, 0, 0, (drive*)ide);
      else{
      }
    }
    ...
    if(stt&1){
      ...
      for(int i=0;i<256/2;i++)*(unsigned short*)&ide->identd[i*2]=io_in16(0x1f0);
      cns->puts("LBA=%x\n", *(unsigned int*)&ide->identd[0x78]);
      if(*(unsigned int*)&ide->identd[0x78]<0xfffffff&&*(unsigned int*)&ide->identd[0x78])drvd::registdrv(1, 0, 1, (drive*)ide);
    }
  }
};
